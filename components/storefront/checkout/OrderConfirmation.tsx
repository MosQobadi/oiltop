...
export function OrderConfirmation({ locale }: { locale: Locale }) {
  ...
  if (!isHydrated) {
    return (
      <p role="status" className="text-[14px] text-fg-subtle">
  ...
  return (
    <div data-testid="order-confirmation">
      ...
      <h1 className="mt-2 text-[27px] font-semibold tracking-[-0.025em] text-fg">
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_340px] lg:gap-8">
        ...
        <aside className="rounded-2xl border border-line bg-surface p-5 lg:sticky lg:top-24 lg:self-start">
          ...
          <Link
            href={navHref(locale, PRODUCTS_PATH)}
            className="focus-visible:ring-accent bg-accent-solid mt-5 flex min-h-11 w-full items-center justify-center rounded-[9px] px-5 text-[14px] font-medium text-white transition-colors hover:bg-accent-solid-hover focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none"
          >
            {pickLocale(locale, "Continue shopping", "ادامه‌ی خرید")}
          </Link>
          <Link
            href={navHref(locale, CART_PATH)}
            className="focus-visible:ring-accent mt-2 flex min-h-11 w-full items-center justify-center rounded-[9px] border border-line-strong px-5 text-[14px] font-medium text-fg-muted transition-colors hover:border-fg-faint focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none"
          >
...
function Detail({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      ...
      <dd className="mt-1 text-[14px] font-medium text-fg">{children}</dd>
    </div>
  );
}
